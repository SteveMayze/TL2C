...
// #include <avr/iom32.h> // For reference....
#include "SMBSlave.h"
...
void SMBusInit(void)
{
	...
	// TWIE    0 // TWIE: TWI Interrupt Enable

    TWSCRA  = (1 << TWEN ) | (1 << TWSME) | (1 << TWDIE) | (1 << TWASIE) ;
	TWSCRB = (1 << TWAA) | (1 << TWCMD1) | ( 1 << TWCMD0 ) ;
}
...
ISR(TWI_VECT)
{
    ...
    // Use the TWI status information to make decisions.
    switch (TWSSRA  & 0xf8)
    {
        case 0x60:      // SLA + W received, ACK returned
        {
            // State should be IDLE when SLA+W is received. If SLA+W is received
            // and state is not IDLE, an error has probably occurred in an earlier
        ...
        case 0xb8:      // Data byte in TWDR transmitted, ACK received.
        {
            ...
            {
#ifdef SMB_SUPPORT_PEC
                TWSD  = smb.pec;     // If PEC is enabled, an ACK is OK here.
#else
    ...
    if (enableACK)
    {
        // Set TWASIE flag and don't clear TWDIE (if set) at the same time.
        temp = TWSCRA ;
        temp |= (1 << TWASIE);
        temp &= ~(1 << TWDIE);
        TWSCRA  = temp;
    ...
    {
        // Clear TWASIE flag and make sure that TWDIE is not cleared at the same time.
        TWSCRA  &= ~((1 << TWASIE) | (1 << TWDIE));
    }

    TWSCRA  |= (1 << TWEN) | (1 << TWEN) | (1 << TWDIE);
}
